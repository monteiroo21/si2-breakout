...
class NStepCollector:
    ...
    def push(self, state, action, reward, next_state, done) -> List[Tuple]:
        ...
        while len(self.buf) >= self.n:
            ...
            self.buf.popleft()
        if done:  # flush remaining shorter (terminal) windows
            while self.buf:
                ready.append(self._make(len(self.buf)))
                self.buf.popleft()
        return ready

    def _make(self, k: int) -> Tuple:
        cum_reward = 0.0
        for i in range(k):
            cum_reward += (self.gamma ** i) * self.buf[i][2]
        state, action = self.buf[0][0], self.buf[0][1]
        last_next_state, last_done = self.buf[k - 1][3], self.buf[k - 1][4]
        return (state, action, cum_reward, last_next_state, last_done)
...
def compute_td_loss(online, target, optimizer, buffer, batch_size, gamma_disc) -> float:
    ...
    with torch.no_grad():
        ...
        target_q = rewards_t + gamma_disc * next_q * (1.0 - dones_t)
...
def train(args) -> None:
    ...
    collector = NStepCollector(args.n_step, GAMMA)
    gamma_disc = GAMMA ** args.n_step

    print(f"[{args.tag}] {args.n_step}-STEP DQN  device={device}  steps={args.timesteps}")
    ...
    for step in range(1, args.timesteps + 1):
        ...
        for tr in collector.push(stacked, action, reward, next_stacked, float(done)):
            buffer.push(*tr)
        ...
        if len(buffer) > args.learning_starts and step % args.train_freq == 0:
            losses.append(compute_td_loss(online, target, optimizer, buffer, args.batch_size, gamma_disc))
...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--n-step", type=int, default=3, help="number of steps to accumulate")
    parser.add_argument("--tag", default="scratch_nstep", help="output folder under models/ and runs/")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="n-step DQN from scratch")
    ...
    if args.eval_only:
        ...
        print_eval("nstep", args.model, args.episodes, peaks, clears, survivals)
        return
