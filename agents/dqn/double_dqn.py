...
def compute_td_loss(online, target, optimizer, buffer, batch_size) -> float:
    ...
    with torch.no_grad():
        next_actions = online(next_states_t).argmax(dim=1, keepdim=True)   # online selects
        next_q = target(next_states_t).gather(1, next_actions).squeeze(1)  # target evaluates
...
def train(args) -> None:
    ...
    print(f"[{args.tag}] DOUBLE DQN  device={device}  steps={args.timesteps}")
...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--tag", default="scratch_double", help="output folder under models/ and runs/")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Double DQN from scratch")
    ...
    if args.eval_only:
        ...
        print_eval("double", args.model, args.episodes, peaks, clears, survivals)
        return
